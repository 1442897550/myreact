...
public class ChunkService {
    ...
    public List<DocChunk> chunkDocument(String content, String filePath){
        ...
        for (Section section : sections) {
            List<DocChunk> sectionChunks = chunkSection(section, globalChunkIndex);
     ...
     * 对单个章节进行分片
     */
    private List<DocChunk> chunkSection(Section section, int startChunkIndex) {
        List<DocChunk> chunks = new ArrayList<>();
        ...
        if (content.length() <= 800) {
            ...
            chunk.setChunkIndex(section.startIndex);
            chunk.setEndIndex(section.startIndex+content.length());
            chunk.setChunkIndex(startChunkIndex);
        ...
        for (String paragraph : paragraphs) {
            ...
            if (currentChunk.length() > 0 &&
                    currentChunk.length() + paragraph.length() > 800) {
    ...
    private String getOverlapText(String text) {
        int overlapSize = Math.min(100, text.length());
        if (overlapSize <= 0) {
            return "";
        }
